<?
      class Contacto_Ventas
	{
	  ...
	  function delete($id)
	  {
		  $sql="delete from CONTACTO_VENTAS where contacto_ventas_id=".$id."";
		  $res=mysql_query($sql);
	  ...
	  function busqueda_contacto_id($search, $cliente)
	  {
		  
		  $search= str_replace(' ', '%', $search);
		  $sql="select CONTACTO_VENTAS.contacto_ventas_id, GENERALES.nombre, GENERALES.apel_p,GENERALES.apel_m from CONTACTO_VENTAS, GENERALES  WHERE CONTACTO_VENTAS.cliente_id=".$cliente."";
		  if(!empty($search))
		  {
		   $sql=$sql." AND (GENERALES.nombre like '%".$search."%' OR GENERALES.apel_p OR GENERALES.apel_m like '%".$search."%')";
		  }
