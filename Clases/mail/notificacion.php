...
require("../pdf/createremision.php");
...
function enviarNotificacionAutorizacion( $id, $estado, $obs, $usu)
{
	...
	$mailer->ponerValores('ssl://smtp.gmail.com','465', true, $arr_conf[2], $arr_conf[3], "Notificaciones");
	if($estado==4)
	{
...
function enviarCotizacion($cot, $usu, $pass, $contacto, $msg)
{
	...
	$arr_conf=$config->detalle();
	$mailer=new Mailer();
	$mailer->ponerValores('ssl://smtp.gmail.com','465', true, "notificaciones@mogel.com.mx", "Mogel2018", $arr_usu[1]." ".$arr_usu[2]);

	$pdf=new PDF();
	    $folio=$pdf->printandsave($cot);
		$nombrepdf="cotizacion".$folio.".pdf";
		$adjuntos=array();
		array_push($adjuntos, "../pdf/tmp/".$nombrepdf);
		$id_ftp=ConectarFTP();
		//VerificarDirectorio("usuario1", $cot);
		//$url="usuario1/$cot/";
		$user=$_SESSION['user'];
		...
		{
			array_push($adjuntos, "../../upload".$archivo);	
		}
		
		
		return $mailer->enviarMail("Notificacion de Cotizacion", $msg, $arr_contact[8], $arr_contact[1]." ".$arr_contact[2], $adjuntos);
	...
	return "OK";
		...
		{
			if (($archivo!=".")&&($archivo!=".."))
			array_push($adjuntos, "../../upload/$user/$cot/".$archivo);	
		}
		
		return $mailer->enviarMail("Notificacion de Cotizacion", $msg, 'rafauri2208@gmail.com', $arr_contact[1]." ".$arr_contact[2], $adjuntos);
		return $mailer->enviarMail("Notificacion de Cotizacion", $msg, $arr_usu[8], utf8_decode($arr_usu[1])." ".utf8_decode($arr_usu[2]), $adjuntos);
...
function enviarOrdenCompra($cot, $usu, $pass, $contacto, $msg)
{
	...
	$arr_conf=$config->detalle();
	$mailer=new Mailer();
	$mailer->ponerValores('ssl://smtp.gmail.com','587', true, "notificaciones@mogel.com.mx", "Mogel2018", $arr_usu[1]." ".$arr_usu[2]);
		$pdf=new PDF();
	    $folio=$pdf->printandsave($cot);
		$nombrepdf="cotizacion".$folio.".pdf";
		$adjuntos=array();
