...
function enviarOrdenCompra($cot, $usu, $pass, $contacto, $msg)
{
	...
	$mailer->ponerValores('smtp.gmail.com','587', true, "notificaciones@mogel.com.mx", "Mogel2018!", $arr_usu[1]." ".$arr_usu[2]);
		...
		//return $mailer->enviarMail("Notificacion de Orden de Compra", $msg, $arr_usu[8], utf8_decode($arr_usu[1])." ".utf8_decode($arr_usu[2]), $adjuntos);
...
function enviar_requisicion($cot, $usu, $pass, $contacto, $msg, $subject)
{
	...
	$mailer->ponerValores('smtp.gmail.com','587', true, "notificaciones@mogel.com.mx", "Mogel2018!", $arr_usu[1]." ".$arr_usu[2]);
		$pdf=new PDF();
	    $folio=$pdf->printandsave($cot);
		$nombrepdf="req".$folio.".pdf";
		$adjuntos=array();
		array_push($adjuntos, "../pdf/tmp/".$nombrepdf);

		
		//return $mailer->enviarMail_requisicion($subject, $msg, $arr_usu[8], utf8_decode($arr_usu[1])." ".utf8_decode($arr_usu[2]), $adjuntos);
	//return $mailer->enviarMail_requisicion($subject, $msg, 'rafauri2208@gmail.com', $arr_contact[1]." ".$arr_contact[2], $adjuntos);
		return $mailer->enviarMail_requisicion($subject, $msg, $arr_contact[8], $arr_contact[1]." ".$arr_contact[2], $adjuntos);
	return "OK";
