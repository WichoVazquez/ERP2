...
console.log("FECHA INI:"+$( "#datepicker1" ).val()+"y FECHA FIN:"+$( "#datepicker2" ).val());
...
if(!isNaN(message))
{
						alert("Se Guardó Satisfactoriamente el Pedido");
	...
	$( "#Mostrar_todo" ).dialog({
      ...
      buttons: {
        "Aceptar": 
					...
					{
						bandera=1;
						console.log("Cambio de Status Bandera en 1");
...
//borrar producto de base de datos DETALLE-COTIZACION
   ...
   var arr={
    accion: "entrega",
    pedido: $("#pedidono").val(), 
    producto: $("#Orden_sumario >tbody >tr").eq(x).find("input[type=checkbox]").val(), 
    cantidad: $("#Orden_sumario >tbody >tr").eq(x).find("td").eq(3).html(), 
    precio: $("#Orden_sumario >tbody >tr").eq(x).find("td").eq(4).html(), 
    observaciones: $("#Orden_sumario >tbody >tr").eq(x).find("td").eq(5).html(), 
    multiplo: 1, 
    tipocot: 0,
    status:15  //para que se vaya la Asignación
   }
   ...
   hilo=$.post(
     ...
     $( "#productos tbody" ).append( "<tr>" +
       ...
       "<td>" + $("#Orden_sumario >tbody >tr").eq(x).find("td").eq(3).html() + "</td>" +
       "<td>" + $("#Orden_sumario >tbody >tr").eq(x).find("td").eq(2).html() + "</td>" + //cantidad
     "</tr>" );
...
*/
        ...
        $("#Orden_sumario >tbody >tr input:checkbox")[x].checked = false;
    ...
    $("#agregar-producto")
      ...
      .click(function() {
      	if ($("#folio_OS").val()==""){
      		alert("Favor de Ingresar  Folio de O.S. , Fecha de Inicio y Fecha Final");
      	}
      	else{
				 ...
				 function(msg) {
					...
					if(aDetalle != null)
					{
						for(x=0;x<(aDetalle.length);x++){
							if (bandera==0)
								{
									console.log("Bandera en 0");
							var mtotal_r=aDetalle[x][2]* aDetalle[x][4];
						...
						"</tr>" );
						   	...
						   	console.log("Bandera en 1");
						   	var mtotal_r=aDetalle[x][2]* aDetalle[x][4];
						   $( "#Orden_sumario tbody" ).append( "<tr>" +
						  "<td><input type='checkbox' name='idcotizacionCheck' value='"+ aDetalle[x][9]+"'></td>" +
						  "<td>" +  aDetalle[x][7]+ "</td>" +
						  "<td>" +  aDetalle[x][8] + "</td>" +
						  "<td>" + aDetalle[x][2] + "</td>" +					  
						  "<td>" + aDetalle[x][4] + "</td>" +					  
...
function saveProducto(varia){//arr{cotizacion,producto, cantidad, precio}
		...
		console.log("El peddo chido:"+$("#pedidono").val());
...
function showResultSucursal(str)
{
	...
	else
	{
		...
		xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		xmlhttp.send("sucursal="+str+"&cliente="+$("#cliente_cot").val());  
	}
}
