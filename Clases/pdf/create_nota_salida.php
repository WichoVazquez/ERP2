...
$y_inical = 66;
	...
	$subtotal=0;
			    ...
			    $x=array(12,20, 32, 55, 80, 180);
				$y_i=$y_inical+5;
				$this->Text($x[0], $y_i,'No.');
				$this->Text($x[1]-2, $y_i,'CODIGO');
				$this->Text($x[2]+2, $y_i,'CANTIDAD');
				$this->Text($x[3], $y_i,'U/MEDIDA');
				...
				for($cont=$inicio;$cont<$fin;$cont++)
				{
					...
					$this->Text($x[1], $y_i," ");
					$this->Text($x[2], $y_i,$this->arreglo_ruta[$cont][5]);
					$this->Text($x[3], $y_i,$this->arreglo_ruta[$cont][9]);
					$this->Text($x[4], $y_i,$this->arreglo_ruta[$cont][3]);
					$this->Text($x[5], $y_i,$this->arreglo_ruta[$cont][10]);
...
$this->Text(10,$y_header, utf8_decode("VERSIÓN::                0"));
	$this->Text(130,$y_header, utf8_decode("FOLIO: ".$this->arreglo[1].""));
...
$this->SetFont('Arial','',7);
$this->Text(30	,$this->final_y, utf8_decode($this->arreglo_ruta_sum[0][6]));
$this->Text(43,$this->final_y, utf8_decode('Y'));
$this->Text(46	,$this->final_y, utf8_decode($this->arreglo_ruta_sum[0][7]));	
$this->SetFont('Arial','B',7);
$this->Text(10,$this->final_y+=4, utf8_decode('CONDUCTOR: '));
$this->SetFont('Arial','',7);
$this->Text(30	,$this->final_y, utf8_decode($this->arreglo_ruta_sum[0][12]));
$this->SetFont('Arial','B',7);
...
*/   
	...
	function printandsave($cot)
	{
			 ...
			 ob_end_clean();
			 $this->AliasNbPages();
			 $tamano=count($this->arreglo_ruta);
			 $cuanto=$tamano/15;	 
			 for($x=0;$x<$cuanto;$x++)
			 {
				 $inicio= $x * 15;
