<?
 ...
 $id_recot=$_GET["id"];//id de la cotizacion a recotizar
 require_once("../Clases/Conexion/conexion_prueba_local.php");
 require_once("../Clases/Objetos/cotizacion.php");
 require_once("../Clases/Objetos/detalle_cotizacion.php");
 $link=conect();
 $cotizacion=new Cotizacion();
 $cotizacion->conexion($link);
 $id=$cotizacion->recotizar($id_recot);// id de la nueva cotizacion
 $detalle=new Detalle_Cotizacion();
 $detalle->conexion($link);
 $detalle->recotizar_productos($id_recot, $id);
 //echo "Error:".$id;
 $array=$cotizacion->detalle($id);

  $user=sesiones_start();
  librerias();
  scripts_head("../Clases/javascript/recotizacion.js");
  encabezado_BIG();
  $descuentos=0;
?>
...
<INPUT type="text" name='empresa' id='empresa' class="text ui-widget-content ui-corner-all"  width="200" <? echo "idempresa='".$array[4]."' value='".$array[14]."'"; ?> readonly/>
<LABEL class="ui-widget" for="cliente">Cliente:</LABEL>
<INPUT type="text" name="cliente" id="cliente" class="text ui-widget-content ui-corner-all"  width="200" <? echo " idcliente='".$array[2]."' value='".$array[13]."'";?> readonly/>
<BR>
<LABEL class="ui-widget" for="contacto" style=" padding-right:22px;">AT'N:</LABEL>
<INPUT type="text" name="contacto" id="contacto" class="text ui-widget-content ui-corner-all"   width="200" <? echo " idcontacto='".$array[11]."' value='".$array[15]."&nbsp;".$array[16]."&nbsp;".$array[17]."'";?> />
  
  <LABEL class="ui-widget" for="datepicker"> Fecha de Cotización: <input name="datepicker" type="text" id="datepicker" />

<BR>
<LABEL class="ui-widget" for="mensaje">Leyenda:</LABEL>
<BR>
<TEXTAREA id="mensaje" style="width:430px;" MAXLENGTH="500"><? echo "".$array[12]."";?></TEXTAREA>
</FIELDSET> 
</FORM>
...
?>
    ...
    <INPUT type="text" name="producto" id="producto" class="text ui-widget-content ui-corner-all" onKeyUp="showResult(this.value)" title="Palabras Clave"  style="width:400px;" IDPRODUCTO="0" value=""/>
    ...
    <INPUT type="text" name="cantidad" id="cantidad" class="text ui-widget-content ui-corner-all"   onKeyUp="calcularMontos(this.value)" style="width:100px;" />
   ...
   <LABEL style="width:50px;" for="preciobase">Precio: $</LABEL>
    ...
    <LABEL for="real">Real</LABEL>
        <BR>
    <BR>
    <LABEL style="width:50px;" for="cantidad-promo">Añadir:</LABEL><INPUT type="text" name="cantidad-promo" id="cantidad-promo" class="text ui-widget-content ui-corner-all"  style="width:100px;"  onKeyUp="calcularPromo(this.value)" readonly/>
    <BR>
...
{ 
        for($renglones=0; $renglones<count($arrdet); $renglones++)
        {
                ...
                "<td>" .$arrdet[$renglones][8]. "</td>". //prefijo
